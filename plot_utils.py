...
def plot_1D_regression(x, y, xtest=None, xtesthat=None):
    x,y = x.detach().cpu(), y.detach().cpu()
    plt.figure(1,(10,5))
    handles,labels = [],[]
    if xtest is not None:
        xtest,xtesthat = xtest.detach().cpu(), xtesthat.detach().cpu()
        for xtesthat_ in xtesthat:
            t, = plt.plot(xtest[:,0], xtesthat_[:,0], '-', color='royalblue')
        handles += [t]
        labels  += ['Function draws']
        plt.ylim([-1.5,3.0])
    d, = plt.plot(x, y, '.', color='firebrick')
    handles += [d]
    labels  += ['Data points']
...
def plot_mnist_predictions(X, zt, Xhat, N=5):
    ...
    for n in range(N):
        for t in range(T):
            ...
            plt.imshow(Xhat_[n,t].squeeze(-1), cmap='gray')
            plt.xticks([]); plt.yticks([])
    plt.tight_layout()
    plt.show()
    plt.close()
