...
class MNIST_Decoder(nn.Module):
    ...
    def forward(self, zt):
        ''' Decodes a set of latent trajectores
            Input:
                zt - [L,N,T,q] latent sequences
            Returns:
                X  - [L,N,T,1,28,28] decoded images
        '''
        [L,N,T,q] = zt.shape
        zt = zt.reshape([L*N*T,q])
        Xhat = self.decoder(zt).reshape([L,N,T,1,28,28]) # L,N,T,nc,d,d
        return Xhat
